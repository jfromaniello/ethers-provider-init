...
module.exports.getProvider = (env) => {
  ...
  if (env.ETHERSCAN_API_KEY) {
    providers.push(
      new ethers.providers.EtherscanProvider(network, env.ETHERSCAN_API_KEY)
    );
  }

  if (env.INFURA_API_KEY) {
    providers.push(
      new ethers.providers.InfuraProvider(network, env.INFURA_API_KEY)
    );
  }
  ...
  if (env.ALCHEMY_API_KEY) {
    providers.push(
      new ethers.providers.AlchemyProvider(network, env.ALCHEMY_API_KEY)
    );
  }
  ...
  if (Object.keys(env).some((s) => s.startsWith("POCKET_"))) {
    const requiredPocketVariables = [
      "POCKET_APPLICATION_ID",
      "POCKET_APPLICATION_SECRET_KEY",
    ];
    ...
    providers.push(
      new ethers.providers.PocketProvider(network, {
  ...
  if (env.ANKR_API_KEY) {
    providers.push(
      new ethers.providers.AnkrProvider(network, env.ANKR_API_KEY)
    );
  }

  if (env.JSON_RPC_URL) {
    providers.push(
      new ethers.providers.JsonRpcProvider(env.JSON_RPC_URL, network)
    );
  }
